...
\texttt{s..} \textit{hex} & changes the least-significant part of current address to \textit{hex}\\
...
\texttt{px}$\{$\texttt{h}$|$\texttt{w}$|$\texttt{q}$\}$ \ldots & hexdump half-words (16 bits) / words (32 bits) / \\ & quad-words (64 bits) \\
